...
```text
Lo que se deberia ver en la consulta es el numero de id, el nombre del cientifico y el correo institucional
```
...
```text
Lo que se veria de la consulta es el numero de id, el nombre del proyecto y la descripción del proyecto
```
...
```text
Se veria el id de dataset, el nombre como ventas_historicas y si la fuente es interna o externa
```
...
```text
[se veria el id del experimento, proyecto y cientifico con su respectiva fecha de ejecución]
```
...
```text
Se deberia ver id del modelo, cada uno con sus respectivos nombres, versiones y sus algoritmos
```
