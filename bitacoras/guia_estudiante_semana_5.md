...
| # | Prueba | Resultado esperado | Resultado real | Mensaje / error exacto | Restricción involucrada | ¿Coincidió con Semana 3/4? | Observaciones |
|---|---|---|---|---|---|---|---|
| 1 | `INSERT` válido `cientifico_datos` + `proyecto` | ERROR |ERROR | Violation of UNIQUE KEY constraint 'UQ__cientifi__107A1DDA23C6A131'. Cannot insert duplicate key in object 'dbo.cientifico_datos'. The duplicate key value is (ana.torres@usta.edu.co).| — | | |
| 2 | `INSERT` `experimento` con `id_proyecto=999` | ERROR |ERROR |The INSERT statement conflicted with the FOREIGN KEY constraint "fk_experimento_proyecto". The conflict occurred in database "datalab", table "dbo.proyecto", column 'id_proyecto' | `fk_experimento_proyecto` | | |
| 3 | `INSERT` válido `experimento` + `modelo` | ERROR |ERROR | The INSERT statement conflicted with the FOREIGN KEY constraint "fk_modelo_experimento". The conflict occurred in database "datalab", table "dbo.experimento", column 'id_experimento' | — | | |
| 4 | `INSERT` `metrica` con `valor=1.5` |ERROR  |ERROR |The INSERT statement conflicted with the CHECK constraint "chk_metrica_valor". The conflict occurred in database "datalab", table "dbo.metrica", column 'valor'. | `chk_metrica_valor` | | |
| 5 | `DELETE` `proyecto` con hijos | ERROR |ERROR | The DELETE statement conflicted with the REFERENCE constraint "fk_experimento_proyecto". The conflict occurred in database "datalab", table "dbo.experimento", column 'id_proyecto'.| `fk_experimento_proyecto` | | |
| 6 | `DELETE` `modelo` con métrica | ERROR |ERROR | The INSERT statement conflicted with the FOREIGN KEY constraint "fk_metrica_modelo". The conflict occurred in database "datalab", table "dbo.modelo", column 'id_modelo'. | `fk_metrica_modelo` | | |
