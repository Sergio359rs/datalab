...
La relación se resolvió como 1:0..1, porque un experimento puede no producir ningún modelo si no es exitoso, pero si tiene éxito puede producir como máximo un modelo.
...
**b)** En el modelo E-R (Semana 1), "relación" significa: Una asociación o vínculo entre dos o más entidades.

**c)** En el modelo relacional (esta semana), "relación" significa: Una estructura formada por filas y columnas que representa una tabla.
...
No. La cardinalidad de una tabla es el número de filas que contiene, mientras que la cardinalidad E-R indica cómo se relacionan las entidades, por ejemplo 1:1, 1:N o N:M.

Anoten cada regla en sus propias palabras a medida que el docente las explica:

**Regla 1 — Entidad → tabla:**

Cada entidad del modelo E-R se convierte en una tabla. Sus atributos se convierten en columnas y su llave primaria identifica cada registro.

**Regla 2 — Relación 1:N:**

La llave primaria del lado 1 se coloca como llave foránea en la tabla del lado N.
		
**Regla 3 — Relación N:M:**

Se crea una tabla puente que contiene las llaves primarias de las dos entidades como llaves foráneas. Generalmente ambas forman la llave primaria compuesta.

**Regla 4 — Atributo multivaluado:**

Se crea una tabla independiente para almacenar los diferentes valores del atributo, relacionándola con la entidad mediante una llave foránea.

**Regla 5 — Entidad débil:**

Se convierte en una tabla cuya identificación depende de la entidad fuerte relacionada, utilizando la llave de la entidad fuerte junto con su clave parcial.


**e)** ¿Por qué DataLab no tiene entidades débiles en su núcleo? ¿Alguna de las seis entidades depende de otra para existir?

Porque las seis entidades principales pueden identificarse de manera independiente. CIENTIFICO_DATOS, PROYECTO, DATASET, EXPERIMENTO, MODELO y METRICA tienen una identificación propia y ninguna necesita depender de otra para existir.
